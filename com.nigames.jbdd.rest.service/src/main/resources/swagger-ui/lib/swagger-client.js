        ...
        for (var i = 0; i < this.parameters.length; i++) {
            ...
            if (typeof args[param.name] !== 'undefined') {
                if (param.in === 'path') {
                    ...
                    if (param.in === 'query' && typeof args[param.name] !== 'undefined') {
                        if (querystring === '') {
                            querystring += '?';
                        } else {
                            querystring += '&';
        ...
        for (i = 0; i < allDefinedParams.length; i++) {
            ...
            if (param.in === 'formData') {
                ...
                if (param.in === 'header' && opts) {
                    ...
                    if (param.in === 'body' && typeof args[param.name] !== 'undefined') {
                        body = args[param.name];
                    }
                }
            }
            ...
            if (typeof obj.type === 'undefined') {
                ...
                if (obj.type === 'array') {
                    ...
                    if (obj.type === 'integer' && obj.format === 'int32') {
                        ...
                        if (obj.type === 'integer' && obj.format === 'int64') {
                            str += 'long';
                        } else {
                            if (obj.type === 'integer' && typeof obj.format === 'undefined') {
                                str += 'long';
                            } else {
                                if (obj.type === 'string' && obj.format === 'date-time') {
                                    ...
                                    if (obj.type === 'string' && obj.format === 'date') {
                                        ...
                                        if (obj.type === 'string' && typeof obj.format === 'undefined') {
                                            str += 'string';
                                        } else {
                                            if (obj.type === 'number' && obj.format === 'float') {
                                                str += 'float';
                                            } else {
                                                if (obj.type === 'number' && obj.format === 'double') {
                                                    str += 'double';
                                                } else {
                                                    if (obj.type === 'number' && typeof obj.format === 'undefined') {
                                                        str += 'double';
                                                    } else {
                                                        if (obj.type === 'boolean') {
                                                            str += 'boolean';
    ...
    typeFromJsonSchema = function (type, format) {
        ...
        if (type === 'integer' && format === 'int32') {
            ...
            if (type === 'integer' && format === 'int64') {
                ...
                if (type === 'integer' && typeof format === 'undefined') {
                    str = 'long';
                } else {
                    if (type === 'string' && format === 'date-time') {
                        str = 'date-time';
                        ...
                        if (type === 'string' && format === 'date') {
                            ...
                            if (type === 'number' && format === 'float') {
                                ...
                                if (type === 'number' && format === 'double') {
                                    ...
                                    if (type === 'number' && typeof format === 'undefined') {
                                        str = 'double';
                                    } else {
                                        if (type === 'boolean') {
                                            str = 'boolean';
    ...
    SwaggerModel.prototype.setReferencedModels = function (allModels) {
        ...
        for (var i = 0; i < this.properties.length; i++) {
            ...
            if (allModels[type]) {
                ...
                if ((property.refDataType) && (allModels[property.refDataType])) {
    ...
    var SwaggerOperation = function (nickname, path, method, parameters, summary, notes, type, responseMessages, resource, consumes, produces, authorizations, deprecated) {
        ...
        this.resource[this.nickname] = function (args, opts, callback, error) {
            ...
            if (typeof args === 'function') {  // right shift 3
                ...
                if (typeof args === 'object' && typeof opts === 'function') { // right shift 2
                    arg1 = args;
                    arg2 = {};
                    arg3 = opts;
                    arg4 = callback;
        ...
        for (var i = 0; i < this.parameters.length; i++) {
            ...
            if (param.paramType === 'header') {
                ...
                if (param.paramType === 'form' || param.paramType.toLowerCase() === 'file') {
                    ...
                    if (param.paramType === 'body' && param.name !== 'body' && typeof args[param.name] !== 'undefined') {
                        if (args.body) {
        ...
        for (i = 0; i < allDefinedParams.length; i++) {
            ...
            if (param.paramType === 'form') {
                ...
                if (param.paramType === 'file') {
                    ...
                    if (param.paramType === 'header' && this.params.headers) {
                        var key = param.name;
    ...
    JQueryHttpClient.prototype.execute = function (obj) {
        ...
        obj.complete = function (response, textStatus, opts) {
            ...
            if (response.status >= 200 && response.status < 300) {
                ...
                if (response.status === 0 || (response.status >= 400 && response.status < 599)) {
                    cb.error(out);
                } else {
        ...
        var transformError = function (error) {
            ...
            if (error.code) {
                ...
                if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
                    // We can tell the client that this should be treated as a missing resource and not as a
                    // permissions thing
                    out.status = 404;
                }
            }
