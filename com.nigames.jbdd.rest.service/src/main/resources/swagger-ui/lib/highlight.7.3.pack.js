var hljs = new function () {
    ...
    function h(p, o) {
        return Array.prototype.map.call(p.childNodes, function (q) {
            if (q.nodeType == 3) {return o ? q.nodeValue.replace(/\n/g, "") : q.nodeValue}
    ...
    function a(q) {
        ...
        for (var o = 0; o < p.length; o++) {if (e[p[o]] || p[o] == "no-highlight") {return p[o]}}
    }
    ...
    function c(q) {
        ...
        (function p(r, s) {
            for (var t = r.firstChild; t; t = t.nextSibling) {
                if (t.nodeType == 3) {s += t.nodeValue.length} else {
                    if (t.nodeName == "BR") {s += 1} else {
                        if (t.nodeType == 1) {
                            o.push({
                                event: "start",
                                offset: s,
    ...
    function j(x, v, w) {
        ...
        function s(A) {
            function z(B) {return " " + B.nodeName + '="' + l(B.value) + '"'}
    ...
    function f(q) {
        ...
        function p(y, w) {
            ...
            if (y.k) {
                ...
                if (typeof y.k == "string") {z("keyword", y.k)} else {
                    for (var x in y.k) {
                        if (!y.k.hasOwnProperty(x)) {continue}
                        z(x, y.k[x])
                    }
                }
                y.k = r
            ...
            if (w) {
                if (y.bWK) {y.b = "\\b(" + s.join("|") + ")\\s"}
                y.bR = o(y.b ? y.b : "\\B|\\b");
                if (!y.e && !y.eW) {y.e = "\\B|\\b"}
                if (y.e) {y.eR = o(y.e)}
                y.tE = y.e || "";
                if (y.eW && w.tE) {y.tE += (y.e ? "|" : "") + w.tE}
            }
            if (y.i) {y.iR = o(y.i)}
            if (y.r === undefined) {y.r = 1}
            if (!y.c) {y.c = []}
            for (var v = 0; v < y.c.length; v++) {
                if (y.c[v] == "self") {y.c[v] = y}
                p(y.c[v], y)
            }
            if (y.starts) {p(y.starts, w)}
            var u = [];
            for (var v = 0; v < y.c.length; v++) {u.push(y.c[v].b)}
            if (y.tE) {u.push(y.tE)}
            if (y.i) {u.push(y.i)}
    ...
    function d(D, E) {
        function o(r, M) {
            for (var L = 0; L < M.c.length; L++) {
                ...
                if (K && K.index == 0) {return M.c[L]}
            }
        }

        function s(K, r) {
            if (K.e && K.eR.test(r)) {return K}
        ...
        } catch (H) {if (H == "Illegal") {return {r: 0, keyword_count: 0, value: l(E)}} else {throw H}}
    }
    ...
    function g(s) {
        ...
        for (var p in e) {
            ...
            var r = d(p, s);
            r.language = p;
            if (r.keyword_count + r.r > q.keyword_count + q.r) {q = r}
    ...
    function i(q, p, o) {
        if (p) {q = q.replace(/^((<[^>]+>|\t)+)/gm, function (r, v, u, t) {return v.replace(/\t/g, p)})}
        if (o) {q = q.replace(/\n/g, "<br>")}
        return q
    }
    ...
    function m(r, u, p) {
        ...
        var s = r.className;
        if (!s.match("(\\s|^)(language-)?" + t + "(\\s|$)")) {s = s ? (s + " " + t) : t}
        r.innerHTML = w.value;
        r.className = s;
